...
public class BoardController {
	...
	/*
	 * @PostMapping("/boardAdd") public String boardAdd(Board board) { // 커맨드 객체 ->
	 * 필드 =name -> setter boardService.addBoard(board);
	 * System.out.println("boardaction 실행"); return "redirect:/boardList"; // 글입력후
	 * "/boardList"로 리다이렉트(재요청) }
	 * 
	 */
	  ...
	  public String boardAdd(Board boardRequest,HttpServletRequest request) throws IllegalStateException, IOException {
	  //1. board안에 fileList 분해하여 DB 들어갈수있는 형태로 만들어야된다. 2. 파일저장 : 파일경로(동적인경로)
	  String path = request.getSession().getServletContext().getRealPath("./upload");
	  boardService.addBoard(boardRequest, path); 
	  return "redirect:/boardList"; 
	  // 글입력후 "/boardList"로 리다이렉트(재요청) 
	  }
